/**
 * AI assistant next-step recommendation domain layer.
 *
...
import { DEFAULT_PROJECTS } from './subjects.js';
...
/** One project recommendation returned by the LLM. */
...
export interface ConchBuildInput {
  ...
  minSegmentMs: number;
  /** Storage-backed projects, including archived entries needed for history. */
  projects?: readonly SubjectDef[];
}

export interface ConchContextResult {
  active: SubjectId[];
  skipped: ConchSkippedEntry[];
  /** Empty when no projects are active. */
  userPrompt: string;
  /** 不含备注的时间线行数（日志/调试用，避免备注全文进日志）。 */
  lineCount: number;
}

/** Aggregate older entries by month once a project exceeds this line count. */
...
/** Build AI assistant context: activity threshold, compact timeline, and prompt. */
export function buildConchContext(input: ConchBuildInput): ConchContextResult {
  ...
  for (const s of projects) bySubject.set(s.id, []);
  const usable = sessions.filter((s) => s.status !== 'voided' && bySubject.has(s.subjectId));
  ...
  const userPrompt =
    active.length === 0
      ...
      : `当前北京时间：${beijingNowLine(nowMs)}\n` +
        ...
        `以下是 ${active.length} 个近期活跃项目的时间线（旧→新），请按要求返回 JSON。\n${skippedNote}\n` +
        blocks.join('\n\n') +
...
/**
 * 解析并清洗 LLM 原始输出。expected is the runtime ordered project catalog.
 * Structural failures (non-JSON / missing subjects array) return null.
 */
export function parseConchLlmOutput(raw: string, expected: readonly SubjectId[]): ConchSubjectRec[] | null {
  ...
  // Preserve the caller's database-backed order; never invent missing projects.
  return expected.flatMap((id) => {
    const rec = recs.find((item) => item.subject_id === id);
    return rec ? [rec] : [];
  });
}
...
export const CONCH_SYSTEM_PROMPT = `你是一个谨慎的时间记录助手。输入是用户项目的
计时记录：起止时间、有效时长、手动备注。你的唯一任务：为每个给出的项目推断
「下一步最应该做什么」。

【推断原则】
1. 先识别每个项目各自的节奏。项目可以是任何工作、学习或团队活动；逐个独立归纳，不要套用。
2. 以该项目最后一条记录为锚点外推一个可执行的下一步，并明确不确定性。
3. 检查近期任务的连续性；若存在显而易见的未完成步骤，可建议补上，并在 rationale 说明依据。
4. 只依据备注中出现过的事实推断。不得编造备注里从未出现的教材名、题集名、题号、
   课程名；引用资料时用用户自己的叫法（如"你一直在做的那本题集"）。
5. next_action 必须具体可立即执行，≤80 个字符。好例子："整理上次会议的行动项"；
   坏例子："继续推进项目"。
6. 若某项目历史太稀疏或规律不明，confidence 给 "low"，给出最稳妥的一般性下一步
...
【返回 schema】
{
  "subjects": [
    {
      "subject_id": "string，与输入一致的项目 id",
      "next_action": "string，具体的下一步",
