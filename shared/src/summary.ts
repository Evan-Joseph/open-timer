 ...
 * projects are passed by the storage-backed caller; defaults exist only for
 * backwards-compatible pure-function tests.
 */
...
import { DEFAULT_PROJECTS, subjectById } from './subjects.js';
...
export interface SummaryInput {
  ...
  minSegmentMs?: number;
  /** Active and archived projects from the database. */
...
export function buildDailySummary(input: SummaryInput): DailySummary {
  ...
  const by_subject: BySubjectEntry[] = projects.map((s) => ({
    subject_id: s.id,
  ...
  const groups = [...new Set(projects.map((p) => p.aggregateGroup).filter((g): g is string => Boolean(g)))];
  const aggregates: AggregateEntry[] = groups.map((group) => ({
    group,
    active_seconds: projects.filter((s) => s.aggregateGroup === group).reduce(
