...
import { QUIET_PERIODS, isQuietMinute, timelineRange } from './timeline-policy.js';

describe('静默时段', () => {
  it('以结构化规则分别描述午饭、午睡、晚饭与晚间洗漱收尾', () => {
  ...
  it('覆盖午饭、午睡、晚饭、晚间洗漱与跨午夜睡眠，边界外立即恢复提醒', () => {
    // 上午专注到 11:20
    ...
    expect(isQuietMinute(21 * 60 + 45)).toBe(true); // 洗漱与浴室关门缓冲
    // 22:30 后夜间休整
