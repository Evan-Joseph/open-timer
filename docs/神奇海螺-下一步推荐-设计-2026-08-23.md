...
**当前选型（2026-08-31 回归定位后）**：`CONCH_API_BASE=https://api.siliconflow.cn/v1`，`CONCH_MODEL=deepseek-ai/DeepSeek-V4-Pro`。这是 2026-08-29 实际使用的高能力模型线路。完整生产等价输入实测约 89 秒，因此服务端超时保持 90 秒，`max_tokens` 保持 2048。
Ling Flash 虽能在约 9 秒返回可解析 JSON，但在完整历史中把“第 6 章题目已完成、收尾题已订正并理解完、当前已进入第 7 章”误读为第 6 章未收尾，已因推断质量不足退役。不再用服务端章节正则和统一 fallback 代替模型的时序理解。
...
成功 200: 见 §6 响应体
错误:     400 INVALID_WINDOW | 401（未登录）| 402 CONCH_QUOTA_EXHAUSTED | 422 LLM_OUTPUT_INVALID
...
不落库，客户端 localStorage（键 `clock-conch-cache-v5`）。命中纪律：同时以专用 `conch_revision` 和当前 `model` 校验；只有已完成时间线与模型均未变时才复用：
