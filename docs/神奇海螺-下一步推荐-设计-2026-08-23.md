...
**当前选型（2026-08-30 核验）**：`CONCH_API_BASE=https://api.siliconflow.com/v1`，`CONCH_MODEL=deepseek-ai/DeepSeek-V4-Pro-0813`。该正式版已替代此前的 V4 Preview；模型标识与 OpenAI 兼容 Chat Completions 用法以 SiliconFlow 官方模型页为准。
...
- **故障关联**：仅记录 `{ event, stage, request_id, elapsed_ms, upstream_status? }`；`stage` 为 `entered`/`upstream_ok`/`upstream_error`/`internal_error`。Safari 出现网络错误时，用户可提供界面显示的短诊断号以和 Worker tail 对照，不能借此记录或展示问题正文。
...
- ~~`CONCH_API_BASE` / `CONCH_MODEL` 用哪家~~ → 已定：SiliconFlow + `deepseek-ai/DeepSeek-V4-Pro-0813`（§4.1 当前核验）。
