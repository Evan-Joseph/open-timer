...
- DeepSeek 官方请求固定 `thinking: { type: 'enabled' }`、`reasoning_effort: 'high'`、`max_tokens: 4096` 与 `response_format: { type: 'json_object' }`；不发送思考模式下不生效的 `temperature`，也不发送 SiliconFlow 专用 `thinking_budget`。官方 JSON Output 为空时只重试一次，两次共享 90 秒总上游预算，第二次仍为空再返回 422。
...
- 成功建议、生成租约和小时额度写 D1。缓存 key 是 `conch_revision + model + window`；租约是条件写入，覆盖 90 秒上游总预算与 D1 收尾。多个标签页或 Worker 同时 miss 时只有一个请求会调用模型，其余得到 `409 CONCH_GENERATING` + `retry_after_ms` 并由客户端等待重试。额度只在真正调用上游前消耗，缓存命中不占额度。
