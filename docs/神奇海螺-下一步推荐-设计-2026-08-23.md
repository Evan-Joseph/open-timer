...
4. **进行中会话**：若某科目正在计时，卡片显示"进行中"徽标，推荐语义为"这一段结束后接着做什么"。
...
**选型结论（2026-08-23 SiliconFlow 实测 bake-off）**：`CONCH_API_BASE=https://api.siliconflow.cn/v1`，`CONCH_MODEL=deepseek-ai/DeepSeek-V4-Pro`。
...
错误:     400 INVALID_WINDOW | 401（未登录）| 422 LLM_OUTPUT_INVALID
          429 RATE_LIMITED | 503 CONCH_NOT_CONFIGURED | 504 LLM_TIMEOUT（60s AbortController）
          502 LLM_UPSTREAM（上游非 200/网络错误，不透传上游细节）
```
...
- `temperature 0.4`，`max_tokens` 留 2048，`response_format: { type: 'json_object' }`（上游不支持则忽略，靠 zod 兜底）。
- **日志纪律**：请求/响应体（含备注全文与 LLM 输出）一律不进日志，沿用"日志不含备注全文"约束。
- 限流用现有 `RateLimiter`（独立实例：60min/20 次，key=clientIp），Worker 重启重置可接受（单用户）。

### 4.3 响应缓存

不落库，客户端 localStorage（键 `clock-conch-cache`）。命中纪律：**以 `/state` 返回的 `revision`（= `maxEventId`）校验**——时间线无任何事件则 revision 不变，反复打开浮层永久命中缓存、零 token 消耗；任何计时事件推高 revision 即失效。兜底：`revision` 不可用（state 未就绪）时按 30 分钟 TTL；7 天安全上限防跨版本漂移。「重新问一次」显式绕过缓存。后续如需跨端共享再加 `conch_cache` 表。

## 5. 结构化输入（server → LLM）

### 5.1 编码原则

- 每个科目一个块；每条会话一行紧凑编码；备注取 `end_note ?? intent_note`（与 `/sessions` API 的 `note` 口径一致）。
...
对 7 个固定科目逐一分类：
- 全时段有效会话数 = 0 → `not_started`（不进 LLM）；
- 最近 7 个北京日（含今日）有效会话数 = 0 → `inactive`（不进 LLM）；
- 其余 → `active`，进 LLM。
...
08-23 16:05 起 · 进行中 0h25m "第6章 微分方程 看课"
...
| `alternative` | 「或者：…」一行 | `--fs-xs`，`--text-3` |
...
完全复用「近 7 天回顾」的居中浮层范式（DESIGN.md §6）：面板 `min(720px, 100vw-64px)`、`max-height: 100dvh-64px` 内滚、遮罩 `--overlay-scrim`、材质 `--popover-surface`、250ms 入场；关闭三通道 Esc / 遮罩 / 右上 X。`conchOpen` 并入 `user_pref` 同步（与 `historyOpen` 同款），多端同开同关。
...
- 客户端缓存命中（同 `window+revision` 且 <30min）直接渲染，标题行显示缓存时间。
...
- D1：单次请求 = 一次索引 range 查询 + segments 查询，同 0003 口径，无全表扫描。
- LLM 调用频率：客户端缓存 + 服务端 20 次/小时限流；`all` 窗口输入约 20–30K token（当前数据量）。
...
- ~~`CONCH_API_BASE` / `CONCH_MODEL` 用哪家~~ → 已定：SiliconFlow + `deepseek-ai/DeepSeek-V4-Pro`（§4.1 实测依据）。
