...
**当前选型（2026-08-31 以凭据模型列表和完整输入核验）**：`CONCH_API_BASE=https://api.siliconflow.cn/v1`，`CONCH_MODEL=inclusionAI/Ling-flash-2.0`。该 Key 的 `/v1/models` 列出该文本 Flash 模型；以当前生产等价输入（106 个有效会话、3 个活跃科目、4154 字符提示词）连续两次实测 8.9 秒、8.6 秒，均返回可解析的 3 科 JSON。模型标识与 OpenAI 兼容 Chat Completions 用法以该凭据所属端点的模型列表为准。
V4-Pro 在同类完整输入上约 89 秒；DeepSeek V4-Flash 单次 20.8 秒且可解析；Step-3.5-Flash 虽在 13 秒返回，但不支持 JSON Mode 且提示词 JSON 未通过 parser。Ling Flash 兼顾结构化输出与低延迟，故采用。`max_tokens` 保持 1024、服务端超时收至 30 秒，避免长时间无反馈。可选思考预算仅在官方顶层 `thinking_budget` 参数可用且实测必要时再启用。
...
错误:     400 INVALID_WINDOW | 401（未登录）| 422 LLM_OUTPUT_INVALID
          429 RATE_LIMITED | 503 CONCH_NOT_CONFIGURED / CONCH_CREDENTIAL_INVALID | 504 LLM_TIMEOUT（30s AbortController）
...
- SiliconFlow Ling Flash 请求固定 `temperature: 0.4`、`max_tokens: 1024`、`response_format: { type: 'json_object' }`；可选思考预算使用顶层 `thinking_budget`，不发送嵌套 `thinking` 对象。
...
不落库，客户端 localStorage（键 `clock-conch-cache-v5`）。命中纪律：同时以专用 `conch_revision`、当前 `model` 和事实校验规则版本校验；只有三者均未变时才复用：
...
3. 信息不足时，给出基于最后一条记录的保守继续动作，confidence 给 "low"；不要用“补缺步”填补不确定性。
...
- 已明确完成、订正或理解完的原始记录不可反驳；缺少记录不是未完成证据。
...
LLM 原始输出经 zod 校验清洗后，服务端再按最近原始备注和已完成章节做事实校验：模型若重推已完成同章练习/视频，或凭空写未完成/未订正，响应退回为低置信的事实型建议。随后由服务端包装为最终响应（`shared/src/conch.ts` 定义类型，前后端共用）：
...
- ~~`CONCH_API_BASE` / `CONCH_MODEL` 用哪家~~ → 已定：SiliconFlow `.cn` 端点 + `inclusionAI/Ling-flash-2.0`（§4.1 当前核验）。
