...
**当前选型（2026-09-01）**：DeepSeek 开放平台，`CONCH_API_BASE=https://api.deepseek.com`，`CONCH_MODEL=deepseek-v4-flash`。官方模型页列出 `DeepSeek-V4-Flash-0731`，与 V4-Pro 共用 JSON Output、思考模式和 1M 上下文；服务端对 Flash 使用 `thinking: { type: 'enabled' }`、`reasoning_effort: 'low'`、`response_format: { type: 'json_object' }` 与 `max_tokens: 2048`，以降低交互等待并保留有限推理。Pro 的 high/4096 作为代码兼容基线保留。
...
- DeepSeek 官方请求固定 `thinking: { type: 'enabled' }`、`response_format: { type: 'json_object' }`；Flash 使用 `reasoning_effort: 'low'` 与 `max_tokens: 2048`，Pro 使用 `reasoning_effort: 'high'` 与 `max_tokens: 4096`；不发送思考模式下不生效的 `temperature`，也不发送 SiliconFlow 专用 `thinking_budget`。每次用户请求最多发出一次上游 POST，45 秒总预算；JSON Output 为空直接返回 422，网络中断或超时不做隐式重发，由用户显式「再问一次」。
...
4. e2e：dev 桩响应下的浮层渲染、跳过科目弱态、开始计划与结束备注隔离。
...
- ~~`CONCH_API_BASE` / `CONCH_MODEL` 用哪家~~ → 当前：DeepSeek 官方端点 + `deepseek-v4-flash`（§4.1 当前核验）；Pro 保留为可回退基线。
- 活动门槛 7 天是否合适（当前：最近 7 个北京日无会话即不推荐）。
- 「开始这个科目」写入 `intent_note`，结束页与时间轴只读显示“开始时计划”；不自动生成结束备注。
