...
**当前选型（2026-08-31 以凭据模型列表和完整输入核验）**：`CONCH_API_BASE=https://api.siliconflow.cn/v1`，`CONCH_MODEL=deepseek-ai/DeepSeek-V4-Flash`。该 Key 的 `/v1/models` 列出该文本 Flash 模型；以当前生产等价输入（106 个有效会话、3 个活跃科目、4154 字符提示词）实测 20.8 秒返回可解析的 3 科 JSON。模型标识与 OpenAI 兼容 Chat Completions 用法以该凭据所属端点的模型列表为准。
V4-Pro 在同类完整输入上约 89 秒，贴近旧 90 秒超时；Flash 的结构化输出通过 parser，因此采用 Flash。`max_tokens` 收至 1024、服务端超时收至 45 秒，避免长时间无反馈。可选思考预算仅在官方顶层 `thinking_budget` 参数可用且实测必要时再启用。
...
- SiliconFlow V4-Flash 请求固定 `temperature: 0.4`、`max_tokens: 1024`、`response_format: { type: 'json_object' }`；可选思考预算使用顶层 `thinking_budget`，不发送嵌套 `thinking` 对象。
...
- ~~`CONCH_API_BASE` / `CONCH_MODEL` 用哪家~~ → 已定：SiliconFlow `.cn` 端点 + `deepseek-ai/DeepSeek-V4-Flash`（§4.1 当前核验）。
