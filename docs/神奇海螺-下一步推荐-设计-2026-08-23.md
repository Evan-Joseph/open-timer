...
此前 SiliconFlow 的 Ling Flash 虽能快速返回 JSON，但在完整历史中把“第 6 章题目已完成、收尾题已订正并理解完、当前已进入第 7 章”误读为第 6 章未收尾，已退役。DeepSeek JSON Output 可能偶发空 `content`；服务端会以同一输入、同一模型和同一 high reasoning 自动重试一次，连续空响应才归类为 `422 LLM_OUTPUT_INVALID`，不伪装成网络错误。
