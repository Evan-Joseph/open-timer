...
export default function App() {
  ...
  if (store.phase === 'bootstrap') {
    return <AuthGate phase="bootstrap" onSetup={store.setupPassword} onLogin={store.login} error={store.error} />;
  }
  if (store.phase === 'login') {
    return <AuthGate phase="login" onSetup={store.setupPassword} onLogin={store.login} error={store.error} />;
  }
  ...
  return (
    <div className="app">
      ...
      <SettingsDialog
        ...
        projects={store.subjects}
        onProjectsChanged={store.refreshProjects}
      />
    </div>
  );
}
