...
export type AuthPhase = 'loading' | 'bootstrap' | 'setup' | 'login' | 'ready';

export interface ClockStore {
  phase: AuthPhase;
  /** Authenticated owner sessions may read and write private timer data. */
  isOwner: boolean;
  subjects: SubjectApi[];
  state: StateApi | null;
...
export function useClockStore(): ClockStore {
  ...
  const refreshProjects = useCallback(async () => {
    // Keep archived metadata locally so historic timeline labels never fall
    // back to raw IDs; action controls filter archived projects themselves.
    const projects = await apiGet<SubjectApi[]>('/api/v1/projects?include_archived=true');
  ...
  useEffect(() => {
    (async () => {
      try {
        const me = await apiGet<{ authenticated: boolean; setup_done: boolean; bootstrap_required?: boolean }>('/api/v1/auth/me');
        if (me.bootstrap_required) setPhase('bootstrap');
        else if (!me.setup_done) setPhase('setup');
        else if (!me.authenticated) setPhase('login');
        else setPhase('ready');
        const subs = me.authenticated ? await apiGet<SubjectApi[]>('/api/v1/projects?include_archived=true').catch(() => [] as SubjectApi[]) : [];
        setSubjects(subs);
      } catch {
        setPhase('login');
      }
