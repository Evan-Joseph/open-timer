...
export async function apiPut<T>(path: string, body: unknown): Promise<{ ok: boolean; status: number; data: T | null }> {
...
export async function apiDelete<T>(path: string): Promise<{ ok: boolean; status: number; data: T | null }> {
