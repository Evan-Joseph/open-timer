...
export default function ClockFace({ store }: { store: ClockStore }) {
  const { state, subjects: allProjects, anchor, busy } = store;
  const subjects = useMemo(() => allProjects.filter((project) => !project.archived_at), [allProjects]);
  const active = state?.active_session ?? null;
  const settings = useSettings();
  /** Keep controls guarded if a future non-owner view is introduced. */
  const readOnly = !store.isOwner;
  ...
  const [selectedSubject, setSelectedSubject] = useState<string>(
    () => localStorage.getItem('clock-last-subject') || subjects[0]?.subject_id || '',
  );
  ...
  // Archived or removed projects must never leave the start control pointing at
  // a stale ID. Prefer the first currently active database project.
  useEffect(() => {
    if (subjects.length > 0 && !subjects.some((subject) => subject.subject_id === selectedSubject)) {
