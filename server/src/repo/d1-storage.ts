...
export class D1Storage implements Storage {
  constructor(
    ...
    private migrations: readonly VersionedMigration[] | string,
  ) {}
  ...
  async migrate(): Promise<void> {
    // Each version is recorded atomically with its SQL. This matters for ALTER TABLE:
    // a new Worker isolate must never replay a previously completed migration.
    await this.db.prepare('CREATE TABLE IF NOT EXISTS schema_migrations (version INTEGER PRIMARY KEY, applied_at_ms INTEGER NOT NULL)').run();
    const { results } = await this.db.prepare('SELECT version FROM schema_migrations').all<{ version: number }>();
    const applied = new Set(results.map((row) => row.version));
    const migrations: readonly VersionedMigration[] =
      typeof this.migrations === 'string' ? [{ version: 1, sql: this.migrations }] : this.migrations;
    for (const migration of migrations) {
      if (applied.has(migration.version)) continue;
      const statements = splitMigrationSql(migration.sql).map((sql) => this.db.prepare(sql));
      statements.push(
        this.db.prepare('INSERT INTO schema_migrations (version, applied_at_ms) VALUES (?, ?)').bind(migration.version, Date.now()),
      );
      await this.db.batch(statements);
    }
    const existing = await this.db.prepare('SELECT id FROM subject LIMIT 1').first<{ id: string }>();
    if (!existing) {
      await this.db.batch(
        DEFAULT_PROJECTS.map((p) =>
          this.db
            .prepare('INSERT INTO subject (id, display_name, aggregate_group, color_id, sort_order, archived_at_ms) VALUES (?, ?, ?, ?, ?, NULL)')
            .bind(p.id, p.displayName, p.aggregateGroup, p.colorId, p.sortOrder),
        ),
      );
    }
  }
  ...
  private rowToProject(r: Record<string, unknown>): import('@clock/shared').SubjectDef {
    return {
      ...
      aggregateGroup: (r.aggregate_group as string | null) ?? null,
      colorId: r.color_id as string,
      sortOrder: r.sort_order as number,
      archivedAtMs: (r.archived_at_ms as number | null) ?? null,
    };
  }
  ...
  async listProjects(includeArchived = false): Promise<import('@clock/shared').SubjectDef[]> {
    ...
    return results.map((r) => this.rowToProject(r));
  }

  async getProject(id: string): Promise<import('@clock/shared').SubjectDef | null> {
    const row = await this.db.prepare('SELECT * FROM subject WHERE id = ?').bind(id).first<Record<string, unknown>>();
  ...
  async createProject(project: import('@clock/shared').SubjectDef): Promise<void> {
    await this.db
      .prepare('INSERT INTO subject (id, display_name, aggregate_group, color_id, sort_order, archived_at_ms) VALUES (?, ?, ?, ?, ?, NULL)')
      .bind(project.id, project.displayName, project.aggregateGroup ?? 'General', project.colorId, project.sortOrder)
      .run();
  }
  ...
  async updateProject(
    ...
    await this.db
      ...
      .bind(patch.displayName, patch.aggregateGroup ?? 'General', patch.colorId, patch.sortOrder, id)
      .run();
  }

  async archiveProject(id: string, nowMs: number): Promise<'archived' | 'not_found' | 'active_session'> {
    if (!(await this.getProject(id))) return 'not_found';
    const active = await this.db
      .prepare("SELECT id FROM session WHERE subject_id = ? AND status IN ('running', 'paused') LIMIT 1")
      .bind(id)
      .first<{ id: string }>();
    if (active) return 'active_session';
    await this.db.prepare('UPDATE subject SET archived_at_ms = ? WHERE id = ?').bind(nowMs, id).run();
    return 'archived';
  }
  ...
  async getAiConfig(): Promise<{ provider: string; apiBase: string; model: string; encryptedApiKey: string; updatedAtMs: number } | null> {
    const row = await this.db.prepare('SELECT * FROM ai_config WHERE id = 1').first<Record<string, unknown>>();
    return row
      ? { provider: row.provider as string, apiBase: row.api_base as string, model: row.model as string, encryptedApiKey: row.encrypted_api_key as string, updatedAtMs: row.updated_at_ms as number }
      : null;
  }

  async setAiConfig(row: { provider: string; apiBase: string; model: string; encryptedApiKey: string; updatedAtMs: number }): Promise<void> {
    await this.db
      .prepare('INSERT INTO ai_config (id, provider, api_base, model, encrypted_api_key, updated_at_ms) VALUES (1, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET provider=excluded.provider, api_base=excluded.api_base, model=excluded.model, encrypted_api_key=excluded.encrypted_api_key, updated_at_ms=excluded.updated_at_ms')
      .bind(row.provider, row.apiBase, row.model, row.encryptedApiKey, row.updatedAtMs)
      .run();
  }
}
