...
export class SqliteStorage implements Storage {
  ...
  private seedProjects(): void {
    const count = this.db.prepare('SELECT COUNT(*) AS count FROM subject').get() as { count: number };
    ...
    const tx = this.db.transaction(() => {
      for (const s of DEFAULT_PROJECTS) insert.run(s.id, s.displayName, s.aggregateGroup, s.colorId, s.sortOrder);
    });
    tx();
  }
  ...
  async updateProject(
    ...
    this.db
      ...
      .run(patch.displayName, patch.aggregateGroup ?? 'General', patch.colorId, patch.sortOrder, id);
  }
  ...
  async archiveProject(id: string, nowMs: number): Promise<'archived' | 'not_found' | 'active_session'> {
    ...
    const active = this.db
      .prepare("SELECT 1 FROM session WHERE subject_id = ? AND status IN ('running', 'paused') LIMIT 1")
      .get(id);
    if (active) return 'active_session';
    this.db.prepare('UPDATE subject SET archived_at_ms = ? WHERE id = ?').run(nowMs, id);
    return 'archived';
  }
  ...
  async adjustSessionStart(sessionId: string, startedAtMs: number, reason: string | null, nowMs: number): Promise<void> {
    ...
    const tx = this.db.transaction(() => {
      ...
      this.db
        .prepare('INSERT INTO audit_log (actor, action, target, detail_json, server_time_ms) VALUES (?, ?, ?, ?, ?)')
        .run('owner', 'session_start_adjust', sessionId, JSON.stringify({ started_at_ms: startedAtMs }), nowMs);
    });
    tx();
  }
  ...
  async getAiConfig(): Promise<{ provider: string; apiBase: string; model: string; encryptedApiKey: string; updatedAtMs: number } | null> {
    const row = this.db.prepare('SELECT * FROM ai_config WHERE id = 1').get() as Record<string, unknown> | undefined;
    return row
  ...
  async setAiConfig(row: { provider: string; apiBase: string; model: string; encryptedApiKey: string; updatedAtMs: number }): Promise<void> {
    this.db
      ...
      .run(row.provider, row.apiBase, row.model, row.encryptedApiKey, row.updatedAtMs);
  }
}
