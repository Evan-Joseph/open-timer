...
import migrationSql0004 from '../../migrations/0004_projects_and_ai.sql';

/** 全部迁移 SQL（按序号拼接；均为幂等，可重复执行）。
 *  注意：新增迁移文件必须在此显式导入，否则 Worker 不会执行
 *  （SQLite 适配器按目录加载，两边加载方式不同，曾因此漏掉 0003）。 */
const migrations = [
  { version: 1, sql: migrationSql0001 },
  { version: 2, sql: migrationSql0002 },
  { version: 3, sql: migrationSql0003 },
  { version: 4, sql: migrationSql0004 },
] as const;
...
interface Env {
  ...
  ASSETS: Fetcher;
  BACKUP?: BackupBucket;
  /** Legacy AI environment settings (optional secrets; absent means no fallback). */
  CONCH_API_BASE?: string;
  CONCH_API_KEY?: string;
  CONCH_MODEL?: string;
  CONCH_THINKING_BUDGET?: string;
  AI_CONFIG_ENCRYPTION_KEY?: string;
  CLOCK_INITIAL_OWNER_PIN?: string;
}
...
export default {
  ...
  async scheduled(_event: ScheduledEvent, env: Env, _ctx: ExecutionContext): Promise<void> {
    if (!env.BACKUP) {
      console.log('backup skipped: no R2 BACKUP binding configured');
      return;
    }
    const nowMs = Date.now();
    try {
      const storage = new D1Storage(env.DB, migrations);
...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  const config: AppConfig = {
    ...
    aiEncryptionKey: env.AI_CONFIG_ENCRYPTION_KEY?.trim() || null,
    initialOwnerPin: /^\d{6}$/.test(env.CLOCK_INITIAL_OWNER_PIN ?? '') ? env.CLOCK_INITIAL_OWNER_PIN! : null,
    version: '0.1.0',
  };
  const storage = new D1Storage(env.DB, migrations);
  ...
  const app = createApp({ storage, config, ...(env.BACKUP ? { backupBucket: env.BACKUP } : {}) });
